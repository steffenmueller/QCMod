...
p := 3;      // prime number p
rho := 2;
N := 25; prec :=60; basis0 := []; basis1 := []; basis2 := []; 
...
if pl gt 1 then print " Computing a symplectic basis of H^1"; end if;
  ...
  data_hol := coleman_data(Q,p,18:useU:=false,basis0:=basis0,basis1:=basis1,basis2:=basis2);
...
repeat 
  ...
  E1_E2_P_vector := Append(Eltseq(E1_E2_P), -factors_local_ht_at_2[i]);  // don't forget about local height at 2
...
until #heights eq g+1 or i gt #ks; 
  ...
  if Dimension(new_E1_E2_subspace) gt Dimension(E1_E2_subspace) then
    ...
    Append(~heights, Evaluate(local_ht_series, loc_coord)); // height of A_Z(b, P)
    Append(~E1_E2_P_vectors, E1_E2_P_vector);
  end if;
  i +:= 1;
until #heights eq g+1 or i gt #ks; 
...
heights_mat := Matrix(g+1,1,[ht : ht in heights]);
height_coeffs := Eltseq(E1_E2_Ps_matrix^(-1)*heights); */
...
assert gammafil_listb_to_z[k] eq 0;
local_height_list[k] := height(PhiAZP0_to_z,betafil, 0,eqsplit,data);
E1_E2_list[k] := E1_tensor_E2(PhiAZP0_to_z,betafil,changebasis,data,Salpha);
global_height := &+[height_coeffs[j,1]*Eltseq(E1_E2_list[k])[j] : j in [1..g]];
...
for P in [P : P in fake1_transformed | Valuation(P[1]) ge 0] do // points not mapped to infinity
  assert &or[are_congruent(Eltseq(P), Eltseq(Q)) : Q in fake2];
end for;
"Sanity check passed";
...
// check that we're not missing any small points
small_pts := Points(X : Bound := 1000); 
...
//Q:=y^2 -( x^5 - x^4 + x^3 + x^2 - 2*x + 1);
//  data_hol := coleman_data(Q,p,20:useU:=false,basis0:=basis0,basis1:=basis1,basis2:=basis2);
//fake_coeffs_mod_pN, rat_coeffs_mod_pN := coefficients_mod_pN(all_fake_rat_pts, good_rat_pts_affine, divisors, base_pt, splitting_indices, data_hol); 
